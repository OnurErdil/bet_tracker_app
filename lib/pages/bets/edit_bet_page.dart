...
class _EditBetPageState extends State<EditBetPage> {
  ...
  void initState() {
    ...
    _sportController = TextEditingController(text: widget.bet.sport);

    final parts = widget.bet.matchName.split(' - ');
    final homeTeam = parts.isNotEmpty ? parts.first.trim() : '';
    final awayTeam = parts.length > 1 ? parts.sublist(1).join(' - ').trim() : '';

    final detectedTeamPath = _findTeamPath(homeTeam) ?? _findTeamPath(awayTeam);

    _countryController = TextEditingController(
      text: detectedTeamPath?['country'] ?? '',
    );
    _leagueController = TextEditingController(
      text: detectedTeamPath?['league'] ?? '',
    );

    _homeTeamController = TextEditingController(text: homeTeam);
    _awayTeamController = TextEditingController(text: awayTeam);
